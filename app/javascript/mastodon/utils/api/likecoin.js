...
export const getNFTPurchaseInfo = ({ iscnId, classId }) => {
  ...
  return axios.get(`${LIKECOIN_API_BASE}/likernft/purchase?${querystring.stringify(
    qsPayload,
  )}`).catch((err) => {
    console.log(err);
  });
...
export const getChainNFTClassListingEndpoint = classId =>{
  return axios.get(`${LIKECOIN_CHAIN_API}/likechain/likenft/v1/nftIsUseListingPrice/${classId}`).catch(err=>{
    console.log(err);
  });
};
