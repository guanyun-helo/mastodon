...
export default class LikePay extends React.PureComponent {
  ...
  pay = () => {
    ...
    window.open(`https://like.co/in/widget/pay?to=${this.props.likerId || 'editorlikersocial'}&amount=${this.state.value}&via=editorlikersocial&fee=1&remarks=${this.state.memo}&state=${JSON.stringify({ user: this.props.account, statusId: this.props.statusId })}&redirect_uri=https://liker.social/home`, '_blank');
  };
    ...
    return (
      <div className={isShow ? 'support-container animate__animated animate__fadeInUp animate__faster' : 'support-container-disappear animate__animated animate__fadeInDown animate__faster'}>
        ...
        {isSupportSuccess ? (<div className='support-status'>
          <div>
            <div className='support-status-animation' />
          </div>
