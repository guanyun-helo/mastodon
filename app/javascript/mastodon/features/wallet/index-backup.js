...
@injectIntl
@withRouter
class Keplr extends ImmutablePureComponent {
  ...
  handleConnection(connection) {
      ...
      this.state.connector.once('account_change', this.handleAccountChange);
  }
  async handleAccountChange(method) {
    const connection = await this.state.connector.init(method);
    this.handleConnection(connection);
  }
  async connect() {
    const connection = await this.state.connector.openConnectWalletModal();
    this.handleConnection(connection);
  }
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      const session = this.state.connector.restoreSession();
